...
export const ClassicTemplate: React.FC<TemplateProps> = ({ data }) => {
    ...
    experience: experience.length > 0 ? (
      <section key="experience">
        ...
        {experience.map(company => (
          <div key={company.id} className={`mb-4 pt-2 ${company.positions.length === 1 ? 'break-inside-avoid' : ''}`}>
            <div className="flex justify-between items-baseline">
              <h3 className="text-xl font-bold text-gray-800">{company.company}</h3>
              <p className="text-base italic text-gray-700">{company.location}</p>
            </div>
            {company.positions.map(pos => (
              <div key={pos.id} className="mt-2 pl-2">
                <div className="flex justify-between items-baseline break-after-avoid">
                  <h4 className="text-base font-bold text-gray-700">{pos.jobTitle}</h4>
                  <p className="text-sm font-light text-gray-600">{pos.startDate} - {pos.endDate}</p>
                </div>
                <DescriptionRenderer text={pos.description} className="mt-0.5 leading-normal [&_ul]:my-1 [&_ul]:space-y-0.5" />
              </div>
            ))}
          </div>
        ))}
      ...
      <section key="education" className="break-inside-avoid">
        ...
        {education.map(edu => (
          <div key={edu.id} className="mb-2 break-inside-avoid">
            ...
            <div className="flex justify-between items-baseline">
                <p className="text-base italic text-gray-700">{edu.school}</p>
      ...
      <section key="skills" className="break-inside-avoid">
        ...
        <div className="flex flex-wrap gap-2 mt-2">
            {skills.map((skill, index) => (
                skill && <span key={index} className="bg-slate-200 text-slate-800 text-xs font-sans font-medium px-2.5 py-0.5 rounded-full">{skill}</span>
            ))}
        </div>
      </section>
    ) : null,
  ...
  return (
    <div className="p-12 md:p-16 font-serif text-gray-800 w-full">
      <header className="flex justify-between items-start border-b-2 border-gray-300 pb-4 mb-4">
        <div className="text-left">
            {personalInfo.name && <h1 className="text-4xl font-bold tracking-wider text-gray-900">{personalInfo.name}</h1>}
            <div className="flex items-center flex-wrap mt-2 text-base text-gray-600 [&>*:not(:first-child)]:before:content-['•'] [&>*:not(:first-child)]:before:mx-2">
              {personalInfo.email && <span>{personalInfo.email}</span>}
              {personalInfo.phone && <span>{personalInfo.phone}</span>}
              {personalInfo.address && <span>{personalInfo.address}</span>}
            </div>
            <div className="flex items-center flex-wrap mt-1 text-base text-blue-700 [&>*:not(:first-child)]:before:content-['•'] [&>*:not(:first-child)]:before:mx-2">
